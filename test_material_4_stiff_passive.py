"""
Test Material 4: Stiff Passive - Pure 5x5x5 cube
...
print("="*70)
print("MATERIAL 4 TEST: Stiff Passive (Blue)")
print("="*70)
print("\n5x5x5 cube - ALL voxels are Stiff Passive")
print("NO actuation - This is the CONTROL test")
print("Expected: NO oscillation (just gravity settling)")
print("="*70)

# Create 5x5x5 cube - ALL Stiff Passive
voxel_grid = np.zeros((8, 8, 8), dtype=np.int8)
for x in range(2, 7):
    for y in range(2, 7):
        for z in range(2, 7):
            voxel_grid[x, y, z] = 4  # All Stiff Passive (blue)
...
print("\n" + "="*70)
print("MATERIAL 4 RESULTS: Stiff Passive (CONTROL)")
print("="*70)
if len(sizes) > 1:
    variation = (max(sizes) - min(sizes)) / np.mean(sizes) * 100
